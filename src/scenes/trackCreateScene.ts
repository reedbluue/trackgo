...
export const trackCreateScene = new Scenes.WizardScene<WizardSceneInterface>(
  'trackCreateScene',
  async (ctx) => {
    ctx.scene.session.userID = ctx.userID;
    await ctx.replyWithHTML(
      `Tudo bem, ${ctx.from.first_name}!  &#x1F44B  Vamos criar uma nova Track!
...
Com dificuldade? Digite <code>/ajuda</code>!`);
      ...
      await ctx.replyWithHTML(
        `&#x2705  Track criada com sucesso!  &#x2705`,
...
trackCreateScene.use(async (ctx, next) => {
  ...
  ctx.scene.session.inativeUserCallBack = setTimeout(async function () {
    await ctx.reply(
      'Inatividade detectada. Saindo do assistente de criação de Tracks.',
      Markup.removeKeyboard()
    );
    await ctx.scene.leave();
  }, 60000);
...
trackCreateScene.command('sair', async (ctx) => {
  await ctx.reply(
    'Tudo bem! Saindo do assistente de criação de Tracks.',
    Markup.removeKeyboard()
  );
  await ctx.scene.leave();
});
