...
export const trackUpdateDescScene =
  new Scenes.WizardScene<WizardSceneInterface>(
...
Digite a nova descrição do Track:`,
        Markup.keyboard([['/ajuda', '/sair']])
      );
      ctx.wizard.next();
    },
    async (ctx) => {
...
Tente novamente mais tarde.`);
        await ctx.replyWithHTML(
          `A descrição do Track foi atualizada com sucesso! 👌`,
...
trackUpdateDescScene.use(async (ctx, next) => {
  ...
  ctx.scene.session.inativeUserCallBack = setTimeout(async function () {
    await ctx.reply(
      'Inatividade detectada. Saindo do assistente de edição.',
      Markup.removeKeyboard()
    );
    await ctx.scene.leave();
  }, 30000);
...
trackUpdateDescScene.command('ajuda', async (ctx) => {
  await ctx.replyWithHTML(
    `A descrição da Track, deve conter uma breve descrição do seu pacote, e deve conter de 5 à 20 caractéres!`
  );
});
...
export const trackUpdateCodeScene =
  new Scenes.WizardScene<WizardSceneInterface>(
...
Com dificuldade? Digite <code>/ajuda</code>!`);
        ...
        if (
          await TrackService.isDuplicate(
            ctx.scene.session.code,
            ctx.scene.session.userID
          )
        ) {
...
Com dificuldade? Digite <code>/ajuda</code>!`);
        ...
        Tente novamente mais tarde.`);
          await ctx.replyWithHTML(
            `O código do Track foi atualizado com sucesso! 👌`,
            Markup.removeKeyboard()
          );
          await ctx.scene.leave();
        }
