...
export abstract class TrackService {
  public static async adicionar(
    ...
    try {
      ...
      const isNotUnique = await TrackService.isDuplicate(
        code,
        <Schema.Types.ObjectId>user
      );
      if (isNotUnique)
  ...
  public static async atualizarTrack(
    trackId: Schema.Types.ObjectId | string,
    keys: Object
  ): Promise<Array<TrackInterface> | undefined | null> {
    try {
      const updateReturn = await TrackDao.update({ _id: trackId }, keys);
      if (!updateReturn || updateReturn.length) return null;
      return updateReturn;
    } catch (err) {
      throw new TrackReadError(err);
    }
  ...
  public static async atualizarTodasAsTracks() {
    try {
      ...
      const tracksToUpdate = allTracks.filter((track) => {
        if (!track.status || !track.status.length) return true;
        return track.status[track.status.length - 1].code !== 'BDE';
      });

      const rawTracks = await TrackerHelper.returnFrom(
        tracksToUpdate.map((tracks) => tracks.code)
      );
      ...
      for (const track of tracksToUpdate) {
        ...
        const statusNotExists =
          (!track.status && !rawTracks[i].status) || !rawTracks[i].status;
        const isNotSameLength =
          track.status?.length == rawTracks[i].status?.length;

        if (same && !statusNotExists && !isNotSameLength) {
          const updatedTrack = await TrackDao.update(
            { _id: track.id },
            rawTracks[i],
            true
          );
